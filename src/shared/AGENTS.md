...
- `CinematicTransition` provides `CinematicTransitionProvider` + `useCinematicTransition().play(variant)` and `lastPlayedAt()`; variants: `immersive` (warp), `unlock` (heart-iris), `seal`, `reveal`. Overlays are z-[120], pointer-events-none, reduced-motion aware. Under rich motion the unlock/seal decor is the lazy `HeartIrisEmbers` WebGL swarm; the SVG rim + hearts remain the fallback (and Suspense placeholder)
